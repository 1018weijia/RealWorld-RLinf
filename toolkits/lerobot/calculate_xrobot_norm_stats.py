...
"""Fast XRobot norm_stats from parquet only (no video decode).

Applies the same 14-D delta-action transform used by ``pi05_xrobot``.
Dataset is LeRobot v2.1 (``episode_XXXXXX.parquet`` per episode); all
episodes are success trajectories (no episode filter).
"""
...
def _iter_episode_parquet(data_root: pathlib.Path):
    # LeRobot v2.1 layout: data/chunk-*/episode_XXXXXX.parquet
    files = sorted(data_root.glob("chunk-*/episode_*.parquet"))
    if not files:
        # Fallback for v3-style shard files if layout changes.
...
def _delta_actions(state_14: np.ndarray, actions_chunk: np.ndarray) -> np.ndarray:
    """Match openpi ``DeltaActions`` for the 14-D EE pose mask."""
...
def main(
    dataset_root: str = "/data/gxy/realworldRL/datasets/XRobot",
    output_dir: str = (
        "/data/gxy/realworldRL/checkpoints/assets/xrobot/put_ring_on_the_rod"
    ),
    ...
    for path, table in tqdm.tqdm(
        ...
        for _ep, (states, actions) in _episode_arrays(table).items():
            ...
            if states.shape[-1] != _XROBOT_ACTION_DIM:
                raise SystemExit(
                    f"{path}: state dim != {_XROBOT_ACTION_DIM}: {states.shape}"
                )
            if actions.shape[-1] != _XROBOT_ACTION_DIM:
                raise SystemExit(
                    f"{path}: action dim != {_XROBOT_ACTION_DIM}: {actions.shape}"
                )
            t = states.shape[0]
            if t == 0:
                continue
    ...
    for key, ns in norm_stats.items():
        ...
        if mean.shape[-1] != _XROBOT_ACTION_DIM:
            raise SystemExit(f"{key} dim != {_XROBOT_ACTION_DIM}")
